...
public interface BoardRepository extends JpaRepository<Board, Long> {
  ...
  //JPQL은 SQL을 추상화한 객체지향 쿼리 언어이다. 테이블이 아닌 엔티티를 대상으로 쿼리함
  //JPQL은 특정 데이터베이스의 SQL을 의존하지 않는다. 모든 데이터베이스에 공통으로 적용.
  //엔티티와 속성은 대소무자를 구분한다.
  //단점: 1) 동적인 쿼리작성에 비효율적, 2)문자열이라서 오류가 있어도 정상작동 함.

  //Board 입장에서 Member를 참조하고 있는 경우, on 생략가능 : Board가 참조 하고 있음
  @Query("select b, w from Board b left join b.writer w where b.bno =:bno")
  Object getBoardWithWriter(@Param("bno") Long bno);

  //Board 입장에서 Reply를 참조하고 있는 경우는 on 생략불가 : Board는 Reply에게 참조 당하고 있음
