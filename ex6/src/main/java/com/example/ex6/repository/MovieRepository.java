...
//  group by m.mno;
  ...
  @Query("select mi2.movie, max(mi2.inum) from MovieImage mi2 group by mi2.movie")
  Page<Object[]> getSubQuery(Pageable pageable);

  @Query("select m, mi,avg(coalesce(r.grade, 0)),count(distinct r) " +
      "from Movie m left outer join MovieImage mi on mi.movie =m " +
      "left outher join Review r on r.moive = m "+
